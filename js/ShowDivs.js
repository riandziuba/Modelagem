...
var Time_Sleep_Caroussel = 10 //defina aki o tempo q demora para o carousel voltar a rolar Automaticamente quando o
 ...
 if(Case == 4){
  z[0].style.opacity = "0"}
 function Next(){set_Time = false;Main_Control(+1);}
 function Prev(){set_Time = false;Main_Control(-1);}
 function Main_Control(v){
   var z = document.getElementsByClassName("icon_button");
   var x = document.getElementsByClassName("Card");
    if(Case == 5 && v=="-1"){
     z[0].style.opacity = "0"}
    else{z[0].style.opacity = "1"}
     if(Case == x.length-1 && v=="+1"){
      z[1].style.opacity = "0"}
    else{ z[1].style.opacity = "1"}
   if(x.length > 4){
   if (Case==x.length && v=="+1") {v=0}Case+=v
   ...
   Main_Function(card += v);}
       ...
       var z = document.getElementsByClassName("icon_button");
       z[0].style.opacity = "0";z[1].style.opacity = "0"
   }}
 ...
 function Auto_RowScrool(){
   var x = document.getElementsByClassName("Card");
   if(Case == x.length){Return = true}
   if(Case == 4){Return = false}
   if(Return == false){
   if(set_Time == true){Main_Control(+1)}
   setTimeout(Aux_Auto_RowScrool, Time_of_Caroussel*1000);}
   else{
   if(set_Time == true){Main_Control(-1)}
   setTimeout(Aux_Auto_RowScrool, Time_of_Caroussel*1000);}}
 function Aux_Auto_RowScrool(){
 if(set_Time == false){set_Time = true;
